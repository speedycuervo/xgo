...
#ONLY PULLING ROOT DISTRIBUTION FOR NOW, BECAUSE CURRENTLY ONLY BUILDING ON LINUX AMD 64
...
#GOOS=linux GOARCH=arm64 CGO_ENABLED=1 CC=aarch64-linux-gnu-gcc-7 go install std
#
#if [ $GO_VERSION -ge 170 ]; then
#  echo "Bootstrapping linux/mips64..."
#  GOOS=linux GOARCH=mips64 CGO_ENABLED=1 CC=mips64-linux-gnuabi64-gcc-7 go install std
#
#  echo "Bootstrapping linux/mips64le..."
#  GOOS=linux GOARCH=mips64le CGO_ENABLED=1 CC=mips64el-linux-gnuabi64-gcc-7 go install std
#fi
#
#if [ $GO_VERSION -ge 180 ]; then
#  echo "Bootstrapping linux/mips..."
#  GOOS=linux GOARCH=mips CGO_ENABLED=1 CC=mips-linux-gnu-gcc-7 go install std
#
#  echo "Bootstrapping linux/mipsle..."
#  GOOS=linux GOARCH=mipsle CGO_ENABLED=1 CC=mipsel-linux-gnu-gcc-7 go install std
#fi
#
